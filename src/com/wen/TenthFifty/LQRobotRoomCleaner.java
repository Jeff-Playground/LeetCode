...
public class LQRobotRoomCleaner {
    interface Robot {
        // returns true if next cell is open and robot moves into the cell.
        // returns false if next cell is obstacle and robot stays on the current cell.
        boolean move();
    ...
    public void cleanRoom(Robot robot){
        ...
        crHelper(robot, 0, 0, 0, visited, dirs);
    }

    // Note here use dir to ensure the computed newX and newY follow the unique direction pattern
    private void crHelper(Robot robot, int x, int y, int dir, Set<String> visited, int[][] dirs){
        robot.clean();
        ...
        for(int i=0; i<4; i++){
            ...
            if(!visited.contains(String.valueOf(newX)+"-"+String.valueOf(newY)) && robot.move()){
                crHelper(robot, newX, newY, cur, visited, dirs);
                robot.turnRight();
                robot.turnRight();
                robot.move();
                robot.turnLeft();
