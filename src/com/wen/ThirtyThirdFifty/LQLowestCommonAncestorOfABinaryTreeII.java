...
public class LQLowestCommonAncestorOfABinaryTreeII {
    ...
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        int[] found=new int[]{0};
        TreeNode result=lcaHelper(root, p, q, found);
        return found[0]==2?result:null;
    }

    private TreeNode lcaHelper(TreeNode node, TreeNode p, TreeNode q, int[] found){
        if(node==null){
            return null;
        } else{
            if(node==p || node==q){
                found[0]++;
                lcaHelper(node.left, p, q, found);
                if(found[0]==1){
                    lcaHelper(node.right, p, q, found);
                }
                return node;
            } else{
                TreeNode left=lcaHelper(node.left, p, q, found);
                if(found[0]==2){
                    return left;
                }
                TreeNode right=lcaHelper(node.right, p, q, found);
                if(found[0]==2){
                    return left!=null?node:right;
                } else if(found[0]==1){
